...
Building this reinforced how much signal is buried in raw logs that you would miss just grepping for keywords. The correlation piece — matching failed and successful logins from the same IP — was the most interesting part because that is exactly the kind of logic SIEM alert rules are built on. It also made me appreciate why log format standardization matters so much. If every application logged differently, none of this automation would work.